...
Fertirriego · Requerimiento: **Zona de equilibrio iónico** (3 col: zona / % meq / kg ventana; no PDF; sí admin/API `ionic_equilibrium`). Dist: editor **solo %**. **Sugerir %** = curva nueva (Steiner). Si ya hay programa, el % se acomoda solo; cambiar una dosis mueve tabla y gráfica. Dinámica: tabla ciclo **% ferti vs % granular** (sin agua). Perfiles → MANUAL §4.6–4.7.
