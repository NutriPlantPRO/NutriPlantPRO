...
OpenAPI: `openapi-nutriplant-admin.json` **v2.12.0** (Import URL: https://nutriplantpro.com/api/admin-assistant/openapi.json). Tras importar, verifica con describe_api → debe responder `version: 2.12.0`.
...
NUTRI PRO: nutri_pro_ask (unified_citations, open_url), nutri_pro_search, nutri_pro_file_text (relations_* archivo↔apunte/link), nutri_pro_reindex (text|ocr), nutri_pro_save — ver NUTRI-PRO-CONOCIMIENTO-GPT
...
NUTRI PRO: preguntas documentos → nutri_pro_ask; inventario → nutri_pro_catalog/search; abrir archivo → open_url; mal indexado → nutri_pro_reindex (ocr si escaneado); corregir texto del MISMO archivo → nutri_pro_set_text (NO nutri_pro_save); leer más → nutri_pro_file_text (acepta open_url; mira relations_*).
