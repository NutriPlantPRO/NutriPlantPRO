...
| Análisis suelo / enmiendas | `soilAnalysis`, `amendments` | meq, pH, ajustes (pestaña Enmiendas) |
| **Análisis laboratorio (6 pestañas)** | `soilAnalyses[]`, `solucionNutritivaAnalyses[]`, `extractoPastaAnalyses[]`, `aguaAnalyses[]`, `foliarAnalyses[]`, `frutaAnalyses[]` | API: `project_analyses` o `project_detail` → `sections.analyses` |
...
| `project_detail` | Resumen completo (incl. `sections.analyses`) | “Dame todo el proyecto X” |
...
| `manual_tecnico_catalog` | **Manual web público** — capítulos, URLs, reglas GEO (sin cuenta) | “¿Qué dice el manual público sobre % meq?” |
| `project_analyses` | **Valores reales** en nube: suelo, solución, pasta, agua, foliar, fruta | “¿Qué foliar tiene el proyecto Limón?” |

**Paquete de actualización GPT (Knowledge + Instructions + OpenAPI 1.9):** ver `docs/CHATGPT-SOCIO-PAQUETE-ACTUALIZACION.md`.  
**Manual público en web:** https://nutriplantpro.com/manual-tecnico/ · Knowledge: `docs/MANUAL-TECNICO-CONOCIMIENTO-GPT.md`
...
3. **Instructions (borrador):**
   ...
   - **Radar NDVI/NDMI:** `radar_project` con `project_name` o `project_id`. Lee `radar_history` (todas las fechas guardadas: `created_at`, `sentinel_period`, `id`). `latest_radar` trae URLs firmadas de la **más reciente**; si el usuario pide una fecha antigua, repite `radar_project` con `params.request_id` = el `id` de esa fila. Siempre di la **fecha** de la imagen que cites. Interpretación: Radar Pilot Copernicus/Sentinel-2 usa colorimetría **relativa al predio**; rojo/naranja = menor nivel relativo, amarillo/verde claro = intermedio, verde/azul verdoso = mayor nivel relativo. No es escala absoluta ni diagnóstico por color. Ver manual cap. `vpd-deficit-presion-vapor`. No analices píxeles: pasa enlaces NDVI/NDMI para que el usuario los abra (~1 h de validez).
4. **Knowledge (obligatorio para criterios):** sube los tres archivos:
   ...
   - `docs/MANUAL-TECNICO-CONOCIMIENTO-GPT.md` (fuente pública del manual web)
5. **Actions** → Import OpenAPI → `docs/openapi-nutriplant-admin.json` (v1.9+) o `/.netlify/functions/nutriplant-admin-assistant`.
