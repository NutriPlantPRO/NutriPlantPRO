...
3. **Instructions (borrador):**
   ...
   - **Radar NDVI/NDMI/NDRE/RGB:** `radar_project` con `project_name` o `project_id`. Lee `radar_history` (fechas: `created_at`, `sentinel_period`, `id`). `latest_radar` trae URLs firmadas de la **más reciente** (índices + RGB si existen); si pide fecha antigua, repite con `params.request_id`. Di siempre la **fecha**. Índices: colorimetría **relativa al predio**. RGB: verde≈planta, rojo/café≈suelo (no Menor/Mayor). Máx. **250 ha**. Ver cap. `vpd-deficit-presion-vapor` / Knowledge MANUAL §4.11. No analices píxeles: pasa enlaces firmados (~1 h).
