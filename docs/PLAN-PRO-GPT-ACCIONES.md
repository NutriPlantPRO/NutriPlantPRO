...
| `plan_pro_item` | Detalle: `item_id` o `q`; incluye **`nutri_refs`**, **`relations_out` / `relations_in`** (grafo tipado) y `also_matched` si hay varios candidatos. Opcional `hops: 2` para un salto más (`hop2_neighbors`). |
