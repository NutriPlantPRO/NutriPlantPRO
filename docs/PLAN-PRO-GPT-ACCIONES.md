...
**Invest PRO** (UI mercados en Plan PRO): ver Knowledge `docs/INVEST-PRO-CONOCIMIENTO-GPT.md`. Cotizaciones = `/api/plan-pro-invest` en la app; **aún no** hay `action` en `nutriplantAdminQuery` para precios. El GPT debe orientar a la pestaña Invest PRO y **no inventar** cotizaciones.
...
| `plan_pro_search` | `q` — palabras sueltas (no hace falta título exacto); ranking por relevancia |
| `plan_pro_item` | Detalle: `item_id` o `q`; incluye **`nutri_refs`**, **`relations_out` / `relations_in`** (grafo apuntes), **`nutri_graph_out` / `nutri_graph_in`** (archivo/link ↔ apunte) y `also_matched`. Opcional `hops: 2`. |
