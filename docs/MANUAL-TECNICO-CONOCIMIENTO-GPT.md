...
**Ajuste por niveles en suelo (UI):** etiqueta «Traer de análisis» + desplegable (reportes Análisis → Suelo). Independiente del agua y de Nutrición granular. Si eliges un reporte: ajuste = max(extracción × **piso%**/100, max(0, extracción total − diferencia considerada del ciclo)). **Piso** (Fertirriego, default 25 %, editable 0–100 junto al desplegable): si el suelo cubre más que la extracción, no se apaga el nutriente (mantenimiento en la zona de riego); luego eficiencia. 0 % = sin piso (puede quedar 0). Si no eliges reporte, se queda como extracción total o lo editado. Editar a mano desvincula. Granular no usa este piso.
