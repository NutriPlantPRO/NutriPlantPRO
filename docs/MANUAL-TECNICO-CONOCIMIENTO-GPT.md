...
**Herramienta 📊 — pasos:** (1) Extracción total kg/ha por nutriente. (2) % por etapa (suma 100 %/nutriente; etapas editables; cada celda con barrita). (3) Resultado kg/ha etapa = total × (%/100). (4) Gráfica macros/micros.
...
**URL:** …/programa-fertirriego-etapas.html · Requerimiento → **Zona de equilibrio iónico** (apoyo, no PDF) → **Distribución objetivo por etapa** (% ) → programa semanal/mensual → aporte agua N-NO₃ → **Dinámica nutricional** (ppm/meq; la lámina se captura en Programa). En Programa, entre columnas de fertilizante hay un **+** para insertar el producto en esa posición (el botón de abajo sigue agregando al final).
...
**Sugerir % vs programa ya hecho:** Sugerir % arma curva nueva (no pisa el programa). Si el proyecto ya tiene dosis, Dist se acomoda sola al aporte real. Si cambias una dosis en Programa (o arrastras en Gráficas), el % de Dist se mueve. Si acabas de pulsar Sugerir % en esa sesión, no se pisa.

**Generador automático:** en Programa, **Propuesta automática de programa** requiere Distribución semanal o mensual y confirmación antes de reemplazar filas. Meta fertilizante etapa = máx(0, requerimiento − agua ciclo − granular/base) × %. El agua y el granular ya vienen descontados en esa meta (no se restan otra vez). Si hay análisis de agua vinculado con HCO₃⁻/CO₃²⁻, el **ácido entra primero** (el que elegiste en el análisis de agua; misma dosis mL/m³ que Hidroponía): L/ha etapa = mL/m³ × **lámina de Programa** (m³/ha de cada periodo) ÷ 1000; sin lámina no hay columna de ácido. La misma lámina se usa en Dinámica para ppm/meq (no se vuelve a capturar ahí). Su N/P/S resta del faltante (no se recorta por el mínimo ~3 kg/ha). Secuencia de sales: **nitrato de calcio primero** (cubre Ca y aporta N) → **MKP para P** (también aporta K) → **MAP solo después de preflor/flor/amarre** (llenado/cosecha) si aún falta P, sin pasarse de N (el NH₄ viene del MAP) → **NKS para el K restante** (aporta N; el SO₄ incidental **no bloquea el K** — si el agua o el ácido ya llenaron S, NKS entra igual y el SO₄ de más se reporta como exceso) → **SOP solo si el N de esa etapa ya se llenó** y aún falta K (tampoco se bloquea porque S ya esté lleno: el K manda; SOP no aporta N) → **N restante: Sulfonit** (si no hay, fosfonitrato). No se usa nitrato de amonio (no es comercial en el agro). **No se usa sulfato de amonio** para forzar S. El S se deja al final: si el granular/agua ya lo cubrieron, **puede quedar pasado**; NKS/SOP/Sulfonit/sulfato de Mg no se bloquean por eso. Luego **una sola fuente de Mg por etapa**: si el N de esa etapa **ya está cubierto**, **sulfato de Mg** (el SO₄ de más se reporta); **nitrato de Mg** solo si aún falta N. No se mezclan las dos sales de Mg en el mismo periodo. **No se usa cloruro de calcio en automático** (cultivos sensibles a Cl⁻); el producto sigue en catálogo por si el agrónomo lo agrega a mano. Si el N se llena antes que el Ca, el Ca que falte queda visible. Luego micros (quelatos, B). Cada dosis se limita por N, P, Ca, Mg… (no sobrepasa esas metas). **Excepción K y Mg:** NKS, SOP y sulfato de Mg pueden pasar SO₄ para cubrir K/Mg; Sulfonit tampoco se bloquea por S. El exceso de S se reporta. Lo demás imposible queda como faltante. El recuadro naranja lista macros y micros (p. ej. Macros: MgO · todo el ciclo), no etapa por etapa. Sales a granel por debajo de ~3 kg/ha por periodo no se incluyen. Micros sí pueden ir en gramos. N-NO₃/N-NH₄ lo determinan los productos. Guarda origen/metas/diagnósticos, reporta **Programa vs distribución** en PDF/admin y marca desactualizado si cambia Distribución. Siempre revisar compatibilidad, solubilidad, CE, agua e inyección.

**Aporte por agua (UI):** etiqueta «Traer de análisis» + desplegable «Seleccionar análisis…» (al elegir un reporte Análisis → Agua se cargan kg/ha **y la leyenda de ácido**, igual que Hidroponía). Los **kg/ha del programa** = ppm × **lámina total del ciclo** (suma de etapas en Programa); si aún no hay lámina, usa el **volumen de referencia del análisis**. Ese campo en Análisis → Agua se etiqueta: **Fertirriego = lámina total del ciclo (m³/ha)**; **Hidroponía = m³ de solución o agua a inyectar**. Las tablas kg del reporte de agua usan ese volumen. El **ácido del programa** no usa ese total: L/ha etapa = mL/m³ × lámina de esa etapa. Si se edita la lámina, esa L/ha y el aporte del ácido se recalculan; la columna queda con **candado cerrado** (Dinámica no la mueve: sale de los meq del agua). En Fertirriego la leyenda muestra **mL/m³** y la **lámina del ciclo** (m³/ha o US gal/acre) con el ácido total; no lista etapa por etapa ni muestra la fórmula. Sin reportes: «Sin análisis de agua en este proyecto». Independiente del ajuste por suelo.
...
- **Ácido:** meq/L a neutralizar = (HCO₃⁻ + CO₃²⁻) − residual; mL/m³ = meq/L×1000÷meq/mL ácido. Ácidos app: HNO₃ 55 %, H₂SO₄ 98 %, H₃PO₄ 75/85 %. No neutralizar 100 % por defecto. En **Hidroponía** y **Fertirriego**, al **Seleccionar análisis…** (etiqueta Traer de análisis) la misma dosis **mL/m³** se muestra como leyenda. En hidroponía entra al **tanque C** (L = mL/m³ × m³ hidro ÷ 1000; ahí sí se compara con el volumen de referencia del análisis si el usuario lo usó como volumen de solución). En fertirriego entra **primero** al programa: L/ha = mL/m³ × lámina etapa (m³/ha) ÷ 1000; la leyenda **no** usa el volumen del análisis (los kg/ha del agua sí: lámina total del ciclo, o el volumen de referencia si aún no hay lámina). Su N/P/S resta del faltante. Admin/PDF; EN si reporte EN. Ver §4.6 y §4.9.  
