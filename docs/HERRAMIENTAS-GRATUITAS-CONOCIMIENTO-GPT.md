...
- Con θ: recuadro **resultado principal** (m³ · mm en franja hasta CC). **Zona objetivo riego:** franja violeta en gráfica = **40–60% del agua útil** (entre PMP y CC); recuadro secundario **hasta objetivo (60% AU)** cuando aplica. Objetivo alto = PMP + 0,6×(CC−PMP). Lámina objetivo = max(0, objetivo alto − θ). Déficit hasta CC = CC − θ. mm en franja = lámina perfil; m³ franja = volumen × (% superficie/100). Publica puente `nutriplant_bridge_soil_water_v1` (incl. m³ objetivo).
...
- **Calculadora** (misma lógica que pestaña **Clima → Lluvia/Riego** del proyecto):
  ...
  - **🪨 Referencia almacén suelo:** desplegable déficit/exceso + m³ manual; botón «Sugerir desde 🪨 suelo» (prioriza objetivo 60% AU); puente `nutriplant_bridge_soil_water_v1`. Solo entra al total si el usuario indica valor.
