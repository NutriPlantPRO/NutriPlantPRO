...
function selectGranularSubTab(tabName) {
  try {
    ...
    if (activeGranularContent && activeGranularContent.id === targetContentId) {
      // Si ya está activa, no salir sin rehidratar.
      // En móviles/retorno desde caché el selector puede perder opciones personalizadas.
      if (tabName === 'requerimiento') {
        loadProjectData();
        requestAnimationFrame(function() {
          try {
            if (typeof window.loadCustomGranularCrops === 'function') {
              Promise.resolve(window.loadCustomGranularCrops()).finally(function() {
        ...
        requestAnimationFrame(function() {
          try {
            if (typeof window.loadCustomGranularCrops === 'function') {
              Promise.resolve(window.loadCustomGranularCrops()).finally(function() {
                if (typeof window.forceLoadApplications === 'function') window.forceLoadApplications();
    ...
    if (tabName === 'requerimiento') {
      // Orden robusto: 1) hidratar proyecto, 2) inyectar cultivos personalizados, 3) render requerimientos.
      // Evita que loadProjectData reemplace el <select> y desaparezcan options custom.
      loadProjectData();
      requestAnimationFrame(function() {
    ...
    // Si se activó el tab de programa, inicializar UI del programa
    if (tabName === 'programa') {
      // Orden robusto igual que requerimiento para no perder catálogo custom.
      loadProjectData();
      requestAnimationFrame(function() {
        ...
        Promise.resolve(customCropsPromiseProg).finally(function() {
          // Poblar Requerimiento (aunque no esté visible) para que updateSummary use valores correctos.
          if (typeof window.loadGranularRequirements === 'function') {
            window.loadGranularRequirements();
          }
...
async function np_loadProjectsFromCloud() {
  ...
  try {
    ...
    // Si en nube ya no existe un proyecto del usuario, quitarlo del local para evitar cache eterna.
    // Se ejecuta siempre que la carga cloud fue exitosa (aunque cloudIds esté vacío).
    if (userId) {
      const userKey = 'nutriplant_user_' + userId;
      var toRemove = [];
      try {
        ...
        // 2) Pasar por todas las claves nutriplant_project_* y quitar las que son de este usuario
        // pero no están en la nube (por si la lista del perfil estaba desactualizada).
        ...
        if (toRemove.length > 0) {
          ...
          var currentId = localStorage.getItem('nutriplant-current-project') || localStorage.getItem('nutriplant_current_project');
    ...
    if (fullData && typeof fullData === 'object') {
      ...
      try {
        ...
        if (localRaw && localRaw.startsWith('{')) {
          ...
          // Regla local-first (con excepción por secciones "ricas"):
          // - Si la nube no trae timestamp confiable, normalmente NO pisar local.
          // - Si la nube es igual o más vieja (margen 1s), normalmente NO pisar local.
          // EXCEPCIÓN: si la nube trae Fertirriego claramente más completo, sí hidratar.
...
function buildReportHydroTriangleSvg(pNO3, pH2PO4, pSO4, pK, pCa, pMg) {
  ...
  // Simple: mitad de cada lado + separacion fija corta.
  const leftMid = lerp(vTop, vLeft, 0.5);
  const rightMid = lerp(vTop, vRight, 0.5);
  const bottomMid = lerp(vLeft, vRight, 0.5);
  const leftEdgeLabel = { x: leftMid.x - 28, y: leftMid.y };
  const rightEdgeLabel = { x: rightMid.x + 28, y: rightMid.y };
  const bottomEdgeLabel = { x: bottomMid.x, y: bottomMid.y + 26 };
  const edgeLabels =
...
window.selectSolucionNutritivaAnalysis = function selectSolucionNutritivaAnalysis(id) {
  ...
  ['Ca','Mg','K','Na'].forEach(function(m) { var k = m.toLowerCase(); var elMeq = document.getElementById('sn-' + k + '-meq'); var elPpm = document.getElementById('sn-' + k + '-ppm'); if (elMeq && a.cations) elMeq.value = snFmt2(a.cations[k + '_meq']); if (elPpm && a.cations) elPpm.value = snFmt2(a.cations[k + '_ppm']); });
  ['NO3','PO4','SO4','Cl','HCO3','CO3'].forEach(function(m) { var k = (m === 'NO3' ? 'no3' : m === 'PO4' ? 'po4' : m === 'SO4' ? 'so4' : m === 'HCO3' ? 'hco3' : m === 'CO3' ? 'co3' : m.toLowerCase()); var elMeq = document.getElementById('sn-' + k + '-meq'); var elPpm = document.getElementById('sn-' + k + '-ppm'); if (elMeq && a.anions) elMeq.value = snFmt2(a.anions[k + '_meq']); if (elPpm && a.anions) elPpm.value = snFmt2(a.anions[k + '_ppm']); });
  if (a.micros) { ['b','fe','mn','zn','cu','mo'].forEach(function(f) { var el = document.getElementById('sn-micro-' + f); if (el) el.value = a.micros[f] !== undefined && a.micros[f] !== '' ? a.micros[f] : ''; }); }
  if (a.ideal) {
    ['b','fe','mn','zn','cu','mo'].forEach(function(f) { var el = document.getElementById('sn-ideal-' + f); if (el) el.value = a.ideal[f] !== undefined && a.ideal[f] !== '' ? a.ideal[f] : ''; });
    ['ca','mg','k','na','no3','po4','so4','cl','hco3','co3'].forEach(function(f) { var el = document.getElementById('sn-ideal-' + f); if (el) el.value = a.ideal[f] !== undefined && a.ideal[f] !== '' ? a.ideal[f] : ''; });
  }
...
window.selectExtractoPastaAnalysis = function selectExtractoPastaAnalysis(id) {
  ...
  ['Ca','Mg','K','Na'].forEach(function(m) { var k = m.toLowerCase(); var elMeq = document.getElementById('ep-' + k + '-meq'); var elPpm = document.getElementById('ep-' + k + '-ppm'); if (elMeq && a.cations) elMeq.value = epFmt2(a.cations[k + '_meq']); if (elPpm && a.cations) elPpm.value = epFmt2(a.cations[k + '_ppm']); });
  ['NO3','PO4','SO4','Cl','HCO3','CO3'].forEach(function(m) { var k = (m === 'NO3' ? 'no3' : m === 'PO4' ? 'po4' : m === 'SO4' ? 'so4' : m === 'HCO3' ? 'hco3' : m === 'CO3' ? 'co3' : m.toLowerCase()); var elMeq = document.getElementById('ep-' + k + '-meq'); var elPpm = document.getElementById('ep-' + k + '-ppm'); if (elMeq && a.anions) elMeq.value = epFmt2(a.anions[k + '_meq']); if (elPpm && a.anions) elPpm.value = epFmt2(a.anions[k + '_ppm']); });
  if (a.micros) { ['b','fe','mn','zn','cu','mo'].forEach(function(f) { var el = document.getElementById('ep-micro-' + f); if (el) el.value = a.micros[f] !== undefined && a.micros[f] !== '' ? a.micros[f] : ''; }); }
...
window.awUpdateSums = function awUpdateSums() {
  ...
  var sumC = 0; ['ca_meq','mg_meq','k_meq','na_meq'].forEach(function(k){ var v = parseFloat(c && c[k]); if (!isNaN(v)) sumC += v; });
  var sumA = 0; ['no3_meq','po4_meq','so4_meq','cl_meq','hco3_meq','co3_meq'].forEach(function(k){ var v = parseFloat(an && an[k]); if (!isNaN(v)) sumA += v; });
...
window.selectAguaAnalysis = function selectAguaAnalysis(id) {
  ...
  ['Ca','Mg','K','Na'].forEach(function(m) { var k = m.toLowerCase(); var elMeq = document.getElementById('aw-' + k + '-meq'); var elPpm = document.getElementById('aw-' + k + '-ppm'); if (elMeq && a.cations) elMeq.value = awFmt(a.cations[k + '_meq']); if (elPpm && a.cations) elPpm.value = awFmt(a.cations[k + '_ppm']); });
  ['NO3','PO4','SO4','Cl','HCO3','CO3'].forEach(function(m) { var k = (m === 'NO3' ? 'no3' : m === 'PO4' ? 'po4' : m === 'SO4' ? 'so4' : m === 'HCO3' ? 'hco3' : m === 'CO3' ? 'co3' : m.toLowerCase()); var elMeq = document.getElementById('aw-' + k + '-meq'); var elPpm = document.getElementById('aw-' + k + '-ppm'); if (elMeq && a.anions) elMeq.value = awFmt(a.anions[k + '_meq']); if (elPpm && a.anions) elPpm.value = awFmt(a.anions[k + '_ppm']); });
  if (a.micros) { ['b','fe','mn','zn','cu'].forEach(function(f) { var el = document.getElementById('aw-micro-' + f); if (el) el.value = a.micros[f] !== undefined && a.micros[f] !== '' ? a.micros[f] : ''; }); }
