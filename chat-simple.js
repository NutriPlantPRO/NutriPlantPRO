...
- Análisis de agua en hidroponía: mismos nutrientes en ppm que el objetivo; el usuario puede **Traer de análisis / Bring from analysis** (selector) para cargar ppm desde un reporte de Análisis → Agua del mismo proyecto (incluido Cl⁻) y restarlo del objetivo antes de fertilizar. N-NH₄⁺ y Mo del agua suelen quedar en 0 si el análisis no los trae.
- **Leyenda de ácido y volumen (obligatorio conocerla):** bajo el bloque Análisis de agua (ppm) aparece una caja con: (1) ácido seleccionado en el análisis (HNO₃ 55%, H₂SO₄ 98%, H₃PO₄ 75% o 85%); (2) mL/m³ = meq necesarios × 1000 / meqPerMl del ácido, donde meq necesarios = (HCO₃⁻ + CO₃²⁻) − residual objetivo (por defecto 1 meq/L); (3) L totales según el volumen (m³) guardado en el análisis; (4) L totales recalculados para el **volumen de agua de hidroponía** (m³ del cálculo); (5) aviso si el volumen de hidroponía **coincide o no** con el m³ del análisis (tolerancia ~0,01 m³); (6) advertencia en rojo de revisar la dosis de ácido. La misma leyenda se muestra en el **panel admin** y en el **reporte PDF** (traducida a inglés si el reporte está en EN). Si no hay análisis vinculado, la UI invita a usar “Traer de análisis”.
- Si el usuario usa **Calcular solución automática**, el ácido del análisis puede entrar al **tanque C** como producto líquido (volumen según mL/m³ × volumen hidro). Orden típico: ácido (C) → nitrato de Ca (A) → MAP/MKP → NKS → nitrato de Mg (A) → SOP → sulfato de amonio → micros → sulfatos al final (el S a menudo queda ligeramente sobre/bajo).
- Catálogo y aportes: el asistente recibe catálogo (precargado + personalizado, % elemental incl. Cl si el material lo tiene) y fertilizantes añadidos con dosis y totales ppm por nutriente. También hay **catálogo de soluciones nutritivas** (literatura Steiner/Hoagland/etc. + soluciones propias del usuario).
