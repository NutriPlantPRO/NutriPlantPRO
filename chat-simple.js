...
- 🪨 Agua en suelo y textura — pestaña Agua: CC y PMP (% volumétrico), profundidad (cm), área (ha), % suelo explorado — superficie/franja (%), humedad actual θ (% vol., opcional). Volumen de suelo (m³) = área_ha × profundidad_cm / 10 (1 ha × 30 cm = 3 000 m³). Capacidad útil (% vol.) = CC − PMP. Agua útil referencia (m³) ≈ volumen_suelo × (CC−PMP)/100 × (% superficie/100). Con θ: **recuadro resultado principal** (m³ · mm en franja, destacado); déficit hasta CC = CC − θ; lámina en perfil (mm) ≈ (déficit/100) × profundidad_cm × 10; volumen en franja (m³) = volumen_total × (% superficie/100); **mm en franja = misma lámina de perfil** (no multiplicar mm × %); referencia ha cultivo (mm) = volumen_franja ÷ (ha × 10). Si θ > CC: exceso m³ (encharcamiento). Publica puente `nutriplant_bridge_soil_water_v1` para 🌧️ balance hídrico. Preset textura USDA: CC/PMP ilustrativos. Pestaña Textura: % arena, limo, arcilla → clase USDA.
- 🌧️ Lámina de riego y balance hídrico (login/dashboard + Clima PRO): mapa/GPS, Open-Meteo ETo/lluvia, periodo 1/7 d (30 d solo PRO). **Riego en franja regada: solo m³** (volumen ya aplicado en el periodo; mm solo en resultados). Balance m³ = déficit m³ cultivo − riego m³ franja. Bloque **🪨 Referencia almacén suelo** (m³ hasta CC desde Agua en suelo; botón Actualizar; ≠ riego aplicado). Recuadro azul «Dato importante — riego en campo»: m³ sugeridos + mm en franja cuando ha cultivo ≠ ha regada. Ej.: 90 m³ = 9 mm ref. 1 ha = 15 mm en 0,6 ha franja. Tablas Kc FAO-56 y % suelo explorado.
