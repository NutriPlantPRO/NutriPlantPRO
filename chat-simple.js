...
- Pilot (pestaña 1): busca primero en **14 días** (hasta 3 pasadas claras; con **1 o 2 ya genera**; nubes ≤35%). Si el predio queda incompleto (&lt;~65% útiles), amplía a **21 d** (≤40%) y luego **30 d** (≤50%) juntando hasta 3 pasadas; **siempre guarda la mejor imagen** (no te deja vacío si hubo cobertura mínima). Mediana por píxel + SCL; si cobertura útil baja (&lt;~15%) no guarda imagen vacía. Muestra fechas satelitales de las escenas usadas. Costo: ≤30 ha=1, >30 ha=2, >100 ha=3 créditos.
...
- Pilot (pestaña 1): busca primero **14 días** (hasta 3 pasadas; con 1 o 2 claras ya genera; nubes ≤35%). Si el predio queda incompleto (&lt;~65% útiles) → **21 d** (≤40%) → **30 d** (≤50%), juntando hasta 3 pasadas y **guardando la mejor**. Mediana por píxel + máscara SCL (nubes/sombra); si cobertura útil &lt;~15% no guarda imagen vacía. Muestra las **fechas satelitales** de las escenas usadas. «Generar / actualizar Pilot», selector Imagen y Ver en mapa. Guarda NDVI+NDMI anclados al polígono (location_snapshot).
- Lectura Satelital (pestaña 2): histórico del mismo predio, 2–6 periodos hacia atrás desde fecha final, quincenal (15 d) o mensual (mes calendario), etiquetas con fechas reales. Por periodo: NDVI/NDMI promedio (píxeles válidos), VPD promedio + horas VPD por banda, ET₀ y lluvia acumulados (Open-Meteo), riego m³↔mm con % franja. Tabla + gráfica + miniaturas NDVI|NDMI (lightbox). Si una quincena queda incompleta (&lt;~65% útiles), solo ese periodo amplía al mes calendario para la imagen (lookback_expanded / expanded_to=mensual); clima/riego siguen en los 15 d. Fechas claras: period_date_* vs search_date_* vs scene_dates. Costo fijo por toda la consulta: 3 créditos Radar (predio ≤30 ha) o 4 (predio >30 ha). Persistido en location.lecturaSatelital. PDF y panel admin muestran tabla, gráfica e imágenes.
...
- Pestaña 1 — Polígono / NDVI y NDMI (Pilot): usa el polígono para generar imágenes Pilot Copernicus/Sentinel-2. Ventana: **14 → 21 → 30 d**; hasta 3 escenas (con **1 o 2 ya genera**), mediana + SCL. NDVI = vigor relativo; NDMI = condición hídrica relativa del dosel/canopia (no humedad exacta del suelo). Colorimetría relativa al predio: rojo/naranja = menor nivel relativo, amarillo/verde claro = intermedio, verde intenso/azul verdoso en NDMI = mayor nivel relativo.
