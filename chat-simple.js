...
- 💧 Diseño de solución nutritiva (herramienta didáctica global): pestañas **1 · Programa del ciclo** (etapas con título, catálogo Steiner/propias, ternario de la activa, gráficas meq/ppm), **2 · Diseño objetivo**, **3 · Aporte fertilizantes**. Desde Dashboard → Solución Nutritiva hay botón «Programa del ciclo» que abre la misma herramienta. Soluciones propias van al **catálogo del usuario** (cualquier proyecto). Distinta de Análisis → Solución Nutritiva (lab).
...
- **Ácido y auto-cálculo:** con análisis vinculado, L ácido (volumen hidro) = mL/m³ × m³ ÷ 1000; entra **primero** en tanque **C** (modo producto / Total producto); su aporte elemental (N/P/S según ácido; cítrico = 0) resta del faltante antes de sales. El usuario también puede fijar **ppm de un elemento** en la celda del nutriente y se calcula el volumen; debajo de L se muestra cuántos meq/L de alcalinidad (HCO₃⁻+CO₃²⁻) neutralizaría esa dosis. UI: etiqueta «Traer de análisis» + desplegable «Seleccionar análisis…» (al elegir se cargan ppm + dosis de ácido).
...
- Contrastar plan de K/Ca/Mg contra diagnóstico de suelo para evitar excesos.`,
      hidroponia: `
- Hidroponía (en menú Proyecto se llama **Solución Nutritiva**; mismo módulo, tag interno **hidroponia** — no es un tag nuevo) tiene dos subsecciones relacionadas: (1) Solución por etapa: etapas, meq/L, **tabla % meq**, ppm, diagrama ternario; N total = N-NO₃⁻ + N-NH₄⁺. Catálogo de soluciones (Steiner/Hoagland/… + propias); botón vacío «Elegir» abre el catálogo. (2) Cálculo de fertilizantes: objetivo (ppm), análisis de agua (ppm), requerimiento = objetivo − agua; volumen (m³), tanque (L), inyección (L/m³); fertilizantes, dosis, tanques A–E, **meq/L + % meq del aporte** y leyenda N/Cl.
...
- Análisis de Agua — Ácido para neutralizar (lógica que el chat debe entender): (1) Residual objetivo (meq/L): es el "colchón" de alcalinidad que se desea dejar en el agua; no se neutraliza todo el HCO₃⁻ y CO₃²⁻, sino que se deja ese residual como buffer. El usuario ajusta este valor (por defecto 1 meq/L). (2) Meq/L de ácido necesarios = (HCO₃⁻ meq/L + CO₃²⁻ meq/L) − residual objetivo; si el resultado es ≤ 0, no se requiere ácido. (3) Ácidos disponibles en la plataforma: Ácido Nítrico 55% (11.6 meq/mL), Sulfúrico 98% (36.7 meq/mL), Fosfórico 75% (12.0 meq/mL), Fosfórico 85% (14.6 meq/mL), Ácido Cítrico Anhidro 99.5% (25.9 meq/mL; C₆H₈O₇, triprótico, densidad ~1.665 kg/L; solo acidifica — no aporta N, P ni K a la solución nutritiva); cada uno tiene ese valor meq/mL con el que se calcula el volumen. (4) Cálculo: meq totales por m³ = meq/L × 1000; mL ácido por m³ = meq_totales_por_m³ / meqPerMl del ácido seleccionado; L ácido (volumen total) = (mL/m³ × m³ agua) / 1000. La plataforma muestra mL ácido/m³ y L total. (5) Si el usuario pregunta cuántos kg de ácido: kg = volumen ácido (L) × densidad del ácido (kg/L). La densidad es propia de cada producto (ej. nítrico 55% ~1.35 kg/L, sulfúrico 98% ~1.84 kg/L, cítrico anhidro ~1.665 kg/L); el chat puede explicar la fórmula y usar densidades típicas o indicar que consulte la etiqueta del ácido si necesita el valor exacto. (6) En Hidroponía → Cálculo de fertilizantes, «Traer de análisis» reutiliza esta misma dosis: leyenda con mL/m³, L del m³ del análisis, L del volumen de hidroponía y aviso si los m³ no coinciden (también en admin y PDF).
...
- Diferencia claramente:
  ...
  getProjectContext() {
    ...
    try {
      ...
      if (hydro) {
        context += '--- HIDROPONÍA / SOLUCIÓN NUTRITIVA (Proyecto; tag hidroponia — Solución por etapa + Cálculo de fertilizantes) ---\n';
        context += 'En el menú este módulo se llama Solución Nutritiva (💧). Interno: hidroponia. Distinto de Análisis → Solución Nutritiva (laboratorio). Las dos subsecciones están relacionadas: Solución por etapa define el objetivo (ppm/meq por etapa); Cálculo de fertilizantes usa ese objetivo, resta el aporte del agua, y calcula fertilizantes y dosis para cubrir el requerimiento.\n';
