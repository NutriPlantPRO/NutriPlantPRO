...
- Pilot (pestaña 1): hasta 3 escenas en ≤30 días, mediana por píxel + SCL; si cobertura útil baja no guarda imagen vacía. Costo: ≤30 ha=1, >30 ha=2, >100 ha=3 créditos.
- Lectura Satelital (pestaña 2): histórico 2–6 periodos hacia atrás (quincenal 15 d o mensual), con NDVI/NDMI promedio, VPD promedio, ET₀ y lluvia acumulados (Open-Meteo) y riego m³ manual. Quincenal nublado → amplía a 30 d. Costo fijo por consulta: 3 créditos (4 si el predio es >30 ha), sin importar el nº de periodos. Gráfica + galería NDVI|NDMI.
...
- Pilot (pestaña 1): hasta 3 pasadas en ≤30 días, mediana por píxel + máscara SCL (nubes/sombra); si cobertura útil &lt;~15% no guarda imagen vacía. «Generar / actualizar Pilot», selector Imagen y Ver en mapa. Guarda NDVI+NDMI anclados al polígono (location_snapshot).
- Lectura Satelital (pestaña 2): histórico del mismo predio, 2–6 periodos hacia atrás desde fecha final, quincenal (15 d) o mensual (mes calendario), etiquetas con fechas reales. Por periodo: NDVI/NDMI promedio (píxeles válidos), VPD promedio (Open-Meteo horario), ET₀ y lluvia acumulados, riego m³ manual. Gráfica de líneas + galería NDVI|NDMI. Quincenal con baja cobertura → amplía auto a 30 d (lookback_expanded). Costo fijo por toda la consulta: 3 créditos Radar (predio ≤30 ha) o 4 créditos (predio >30 ha), sin importar el nº de periodos. Persistido en location.lecturaSatelital.
...
- Pestaña 1 — Polígono / NDVI y NDMI (Pilot): usa el polígono para generar imágenes Pilot Copernicus/Sentinel-2 (hasta 3 escenas ≤30 d, mediana + SCL). NDVI = vigor relativo; NDMI = condición hídrica relativa del dosel/canopia (no humedad exacta del suelo). Colorimetría relativa al predio: rojo/naranja = menor nivel relativo, amarillo/verde claro = intermedio, verde intenso/azul verdoso en NDMI = mayor nivel relativo.
- Pestaña 2 — Lectura Satelital: histórico 2–6 periodos (quincenal/mensual) con NDVI/NDMI promedio, VPD, ET₀, lluvia y riego m³ manual; gráfica y galería. Costo fijo por consulta: 3 créditos (4 si el predio es >30 ha). Si el contexto trae filas de Lectura, interpreta tendencias y correlaciones (no causa única).
