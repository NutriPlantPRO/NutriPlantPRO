...
- Pilot (pestaña 1): junta hasta **8 pasadas** (mediana) en **14 → 21 → 30 → 45 d** para maximizar cobertura. Solo corta antes si llega ~**100%** útiles; si no, sigue buscando más pasadas (umbral de nubes más amplio) y al final **guarda la mejor** (ej. 89%). Con 1 pasada ya puede generar. SCL; si &lt;~15% no guarda vacío. Costo: ≤30 ha=1, >30 ha=2, >100 ha=3 créditos.
...
- Pilot (pestaña 1): junta hasta **8 pasadas** en **14 → 21 → 30 → 45 d** para maximizar el predio. Solo para si ~**100%** útiles; si no, sigue y **guarda lo mejor**. Mediana + SCL; &lt;~15% no guarda vacío. «Generar / actualizar Pilot», selector Imagen y Ver en mapa. Guarda NDVI+NDMI anclados al polígono (location_snapshot).
- Lectura Satelital (pestaña 2): histórico del mismo predio, 2–6 periodos hacia atrás desde fecha final, quincenal (15 d) o mensual (mes calendario), etiquetas con fechas reales. Por periodo: NDVI/NDMI promedio (píxeles válidos), VPD promedio + horas VPD por banda, ET₀ y lluvia acumulados (Open-Meteo), riego m³↔mm con % franja. Tabla + gráfica + miniaturas NDVI|NDMI (lightbox). Hasta **6 pasadas** por periodo (mediana) para maximizar; solo para si ~100% útiles; si quincena incompleta, ese periodo amplía al mes (lookback_expanded); clima/riego siguen en 15 d; **guarda lo mejor**. Fechas: period_date_* vs search_date_* vs scene_dates. Bloques (runs) al regenerar. Costo fijo: 3 créditos (≤30 ha) o 4 (>30 ha). Persistido en location.lecturaSatelital.
...
- Pestaña 1 — Polígono / NDVI y NDMI (Pilot): polígono → Pilot Copernicus/Sentinel-2. Ventana **14 → 21 → 30 → 45 d**; hasta **8 escenas** (mediana + SCL) para maximizar; solo corta si ~100% útiles, si no guarda lo mejor. NDVI = vigor relativo; NDMI = condición hídrica relativa del dosel. Colorimetría relativa al predio.
