...
- Pilot (pestaña 1): busca primero en **14 días** (hasta 3 pasadas claras; con **1 o 2 ya genera**; nubes ≤35%), si no hay cobertura útil → **21 d** (≤40%), último recurso → **30 d** (≤50%). Mediana por píxel + SCL; si cobertura útil baja no guarda imagen vacía. Muestra fechas satelitales de las escenas usadas. Costo: ≤30 ha=1, >30 ha=2, >100 ha=3 créditos.
...
- Pilot (pestaña 1): busca primero **14 días** (hasta 3 pasadas; con 1 o 2 claras ya genera; nubes ≤35%), si falta cobertura → **21 d** (≤40%), último recurso → **30 d** (≤50%). Mediana por píxel + máscara SCL (nubes/sombra); si cobertura útil &lt;~15% no guarda imagen vacía. Muestra las **fechas satelitales** de las escenas usadas. «Generar / actualizar Pilot», selector Imagen y Ver en mapa. Guarda NDVI+NDMI anclados al polígono (location_snapshot).
- Lectura Satelital (pestaña 2): histórico del mismo predio, 2–6 periodos hacia atrás desde fecha final, quincenal (15 d) o mensual (mes calendario), etiquetas con fechas reales. Por periodo: NDVI/NDMI promedio (píxeles válidos), VPD promedio + horas VPD por banda, ET₀ y lluvia acumulados (Open-Meteo), riego m³↔mm con % franja. Tabla + gráfica + miniaturas NDVI|NDMI (lightbox). Quincenal y mensual: si baja cobertura → reintenta escenas más claras y puede ampliar a 30 d (lookback_expanded). Costo fijo por toda la consulta: 3 créditos Radar (predio ≤30 ha) o 4 (predio >30 ha). Persistido en location.lecturaSatelital. PDF y panel admin muestran tabla, gráfica e imágenes.
      ...
      if (rows.length) {
        ...
        rows.forEach(function (r) {
          const parts = [
            ...
            r.lookback_expanded ? 'ampliada-a-30d' : null
          ].filter(Boolean);
