...
6B) RADAR SATELITAL (NDVI / NDMI + Lectura Satelital)
...
- Pilot (pestaña 1): busca primero en **14 días** (hasta **6 pasadas**; con 1 ya genera; nubes ≤35%). Si el predio queda incompleto (&lt;~85% útiles), amplía a **21 d** (≤40%) y luego **30 d** (≤50%) juntando hasta 6 pasadas; **siempre guarda la mejor imagen**. Mediana por píxel + SCL; si cobertura útil baja (&lt;~15%) no guarda imagen vacía. Muestra fechas satelitales de las escenas usadas. Costo: ≤30 ha=1, >30 ha=2, >100 ha=3 créditos.
- Lectura Satelital (pestaña 2): histórico 2–6 periodos (quincenal 15 d o mensual), NDVI/NDMI promedio, VPD, horas VPD, ET₀ y lluvia acumulados (Open-Meteo), riego m³↔mm con % franja. Si nubes: reintenta escenas más claras; **quincenal y mensual** pueden ampliar a 30 d (*). Costo fijo por consulta: 3 créditos (4 si predio >30 ha). Tabla + gráfica + miniaturas NDVI/NDMI (también en PDF/admin).
...
- Si el contexto indica última imagen, Lectura Satelital o historial, puedes responder sobre el estado del Radar del proyecto. Si no hay imagen, indicar que debe guardar/sincronizar el predio y generar/actualizar Radar Pilot.
...
function getRadarCultivoManual() {
  return `
RADAR SATELITAL (NDVI / NDMI + Lectura Satelital) — NutriPlant PRO:
- Módulo del dashboard: **Radar Satelital** (interno Ubicación). Dos pestañas: (1) Polígono / NDVI y NDMI; (2) Lectura Satelital.
- Qué es: imágenes satelitales Pilot Copernicus/Sentinel-2 (~10 m) recortadas al polígono del predio. Google Earth Engine queda como respaldo/standby. La escala es relativa dentro del lote (Bajo → Alto), no valores absolutos universales.
...
- Pilot (pestaña 1): busca primero **14 días** (hasta **6 pasadas**; con 1 ya genera; nubes ≤35%). Si el predio queda incompleto (&lt;~85% útiles) → **21 d** (≤40%) → **30 d** (≤50%), juntando hasta 6 pasadas y **guardando la mejor**. Mediana por píxel + máscara SCL (nubes/sombra); si cobertura útil &lt;~15% no guarda imagen vacía. Muestra las **fechas satelitales** de las escenas usadas. «Generar / actualizar Pilot», selector Imagen y Ver en mapa. Guarda NDVI+NDMI anclados al polígono (location_snapshot).
- Lectura Satelital (pestaña 2): histórico del mismo predio, 2–6 periodos hacia atrás desde fecha final, quincenal (15 d) o mensual (mes calendario), etiquetas con fechas reales. Por periodo: NDVI/NDMI promedio (píxeles válidos), VPD promedio + horas VPD por banda, ET₀ y lluvia acumulados (Open-Meteo), riego m³↔mm con % franja. Tabla + gráfica + miniaturas NDVI|NDMI (lightbox). Hasta **6 pasadas** por periodo (mediana); si una quincena queda incompleta (&lt;~85% útiles), solo ese periodo amplía al mes calendario para la imagen (lookback_expanded / expanded_to=mensual); clima/riego siguen en los 15 d. Fechas claras: period_date_* vs search_date_* vs scene_dates. Guarda varios **bloques** (runs) al regenerar: selector «Bloques guardados» para ver lecturas/imágenes pasadas (hasta 8). Costo fijo por toda la consulta: 3 créditos Radar (predio ≤30 ha) o 4 (predio >30 ha). Persistido en location.lecturaSatelital. PDF y panel admin muestran tabla, gráfica e imágenes del bloque activo.
- Cómo usarlo bien: (1) detectar zonas heterogéneas para recorrer y muestrear en campo; (2) cruzar NDVI (vigor) con NDMI (dosel), riego, textura, drenaje, suelo, foliar, plagas y VPD; (3) en Lectura, correlacionar tendencias NDVI/NDMI con VPD, ET₀, lluvia y riego registrado. No recomendar fertilizar o regar solo por el color del mapa.
...
- Pestaña 1 — Polígono / NDVI y NDMI (Pilot): usa el polígono para generar imágenes Pilot Copernicus/Sentinel-2. Ventana: **14 → 21 → 30 d**; hasta **6 escenas** (con 1 ya genera), mediana + SCL; meta ~85% útiles. NDVI = vigor relativo; NDMI = condición hídrica relativa del dosel/canopia (no humedad exacta del suelo). Colorimetría relativa al predio: rojo/naranja = menor nivel relativo, amarillo/verde claro = intermedio, verde intenso/azul verdoso en NDMI = mayor nivel relativo.
- Pestaña 2 — Lectura Satelital: histórico 2–6 periodos (quincenal/mensual) con NDVI/NDMI promedio, VPD, ET₀, lluvia y riego; gráfica y miniaturas. Quincenal y mensual pueden ampliar a 30 d si hay nubes. Costo fijo por consulta: 3 créditos (4 si el predio es >30 ha). Si el contexto trae filas de Lectura, interpreta tendencias y correlaciones (no causa única).
