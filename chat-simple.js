...
- Granular: aplicaciones con materiales (fertilizantes) y % en mezcla; dosis kg/ha por aplicación; aporte = dosis × composición del material. Unidades en óxidos (N, P₂O₅, K₂O, CaO, MgO, S, SO₄) o elemental según modo. **Costo:** precio canónico USD/t × kg producto/ha → **USD/ha** (US customary: USD/acre).
- Fertirriego: programa por semanas; inyectores con concentración o kg/ha por semana; aporte del agua (análisis de agua) se resta del requerimiento. Conversiones óxido↔elemental: P₂O₅→P ×0.436; K₂O→K ×0.83; CaO→Ca ×0.715; MgO→Mg ×0.603. **Costo:** suma por columna de fertilizante (kg o L×densidad) × precio USD/t → **USD/ha** del programa.
- Hidroponía: concentración en ppm (mg/L) por elemento; volumen de tanque y relación de inyección (L stock/m³ agua) para pasar de ppm a kg de fertilizante. Ácido del análisis (mL/m³) → L totales = mL/m³ × m³ hidro ÷ 1000, tanque C; aporte N/P/S del ácido resta del faltante. **Costo:** kg eq del lote × precio USD/t → **USD del lote** (no por ha). Precios solubles compartidos hidro↔ferti (perfil + overrides).
...
- Macros en % materia seca (N, P, K, Ca, Mg, S); micronutrientes en ppm (Fe, Mn, Zn, Cu, B, Mo). Óptimos dependen de especie y etapa; en NutriPlant son editables por análisis.
...
6B) RADAR SATELITAL (NDVI / NDMI / NDRE / RGB + Lectura Satelital)
...
- Pilot (pestaña 1): elige **1 sola pasada** (la más clara sobre el predio; sin mediana ni relleno entre fechas) en **14 → 21 → 30 → 45 d**. Solo corta si ~**100%** útiles; si no, guarda lo mejor que den las pasadas (≥~5%). SCL. Genera las 4 capas juntas. Costo: ≤30 ha=1, >30 ha=2, >100 ha=3 créditos.
- Lectura Satelital (pestaña 2): histórico 2–6 periodos (quincenal/mensual), NDVI/NDMI/NDRE promedio, VPD, ET₀, lluvia, riego; miniaturas NDVI|NDMI|NDRE|RGB. **1 pasada**/periodo (la más clara; sin mediana); quincena incompleta puede ampliar al mes (*). Costo fijo: 3 créditos (4 si >30 ha). Mismo tope 250 ha. También en PDF/admin.
...
- Un ppm de P/micros/B no es universal: hay que conocer el extractante. Si el lab no lo dice, pregúntalo o usa el método guardado en el reporte y decláralo. No compares Bray vs Olsen ni DTPA vs Mehlich 3 como la misma escala.
- P (ppm) ideal: selector propio (Bray 40, Olsen 25, Merich/Mehlich 3 → 40). Al cambiar pisa el ideal de P; sigue editable.
- Fe/Mn/Zn/Cu: un selector en Fe (DTPA 20/20/3/1,5 · Merich 50/20/3/2 · Otro no pisa). Aplica a los cuatro.
- B: selector propio (agua caliente 1 · Merich 1,2 · Otro no pisa). El B no va con DTPA.
- MO, N-NO3, Na, S, Mo, Al: ideales generales fijos (botón Recargar). K/Ca/Mg salen de CIC, no del extractante de P/micros.
...
/** Manual fijo de calculadoras PRO del dashboard (💧 solución didáctica, 🔗 interacciones, 🧬 aminoácidos, 🪨 agua/textura, 🧂 solubilidad/IS). */
...
- 🌧️ Lámina de riego y balance hídrico (login/dashboard + Clima PRO): mapa/GPS, Open-Meteo ETo/lluvia, periodo 1/7 d (30 d solo PRO). **Riego en franja regada: solo m³** (volumen ya aplicado; mm solo en resultados). Balance m³ = déficit m³ cultivo − riego m³ franja. Bloque **🪨 Referencia almacén suelo:** desplegable Sin ajuste / Déficit (+ riego) / Exceso (− riego) + m³ manual; **«Sugerir hasta 60% AU»** o **«Sugerir hasta CC»** (si ≥ 60% AU sin exceso no rellena; si **exceso sobre CC** rellena Exceso − m³ y resta del total); **total integrado (clima ± almacén)** solo si hay m³. Recuadro azul «Dato importante — riego en campo»: m³ + mm en franja. Ej.: 10 m³ en 0,5 ha = 2 mm franja = 1 mm ref. 1 ha. PDF Clima puede incluir bloque 🪨 suelo. Tablas Kc FAO-56 y % suelo explorado. **≠ ISH** (techo hídrico de ciclo).
- 📈 **Rendimiento hídrico (ISH)** (login/dashboard + Clima PRO pestaña Rendimiento hídrico): \`ISH = 100×[1−Σ(D+Fp·E)/ΣETc]\` por **semanas** (máx. 52). Lluvia/ET₀ satélite o manual, Kc, Fp default 0,25, riego opcional mm↔m³/ha + % efectivo. Curva de techo de rendimiento por agua (**solo baja**). Orientativo ≥85 / 70–85 / &lt;70 %; **no** predice cosecha comercial. Persistencia LS / \`climateAnalysis.ish\`. Misma física free+PRO. **≠ lámina** (m³ en 1/7/30 d).
- 🎯 **Uniformidad de riego** (login/dashboard, icono 🎯): varios **lotes** con título editable y **Muestra 1, 2, 3…**. Unidad al inicio: caudal (L/h o gph), volumen captado (mL/cm³/L/fl oz/gal + minutos) o lámina (mm/in). Campo: caudal individual y medio, **DU 25%** (media del cuarto bajo / media), **CU Christiansen**, CV, goteros/zonas de bajo caudal, agua en mm y m³/ha (1 mm = 10 m³/ha) **o in y US gal/acre** si el usuario está en US customary; fertirriego kg/ha o lb/acre en zona baja/media/alta (**el nutriente hereda la DU**: cuarto bajo ≈ dosis × DU/100). Diseño: **EU Keller–Karmeli** \`EU = 100×(1−1.27×CVf/√ep)×(qmin/q̄)\`; q = qn×(P/Pn)^x; presiones **MCA o PSI** y caudal **L/h o gph** según preferencias. **UI ES/EN** + métrico/US (física interna SI). Bandas DU ≥90 / 80–90 / 70–80 / &lt;70. ≥16 goteros/lote recomendado. Persistencia \`nutriplant_free_uniformidad_riego_v1\`. **≠** lámina 🌧️ **≠** ISH 📈 **≠** pulso ⏱️.
...
- 💧 Diseño de solución nutritiva (\`hidro-solucion-free.html\`): pestañas **1 · Programa del ciclo** (etapas con título, catálogo Steiner/Hoagland/propias, ternario de la activa, gráficas meq/ppm; tip macros/micros), **2 · Diseño objetivo**, **3 · Aporte fertilizantes**. Desde Dashboard → Solución Nutritiva el botón «Programa del ciclo» abre la misma UI en modal (iframe). En PRO, el programa se guarda en el proyecto como **\`hidroponia.cycleProgram\`** (nombre, etapas meq/ppm, etapa activa) y aparece en **admin** y como sección opcional del PDF **\`hydroCycle\` / Programa del ciclo** (independiente de Solución por etapa + Cálculo de fertilizantes). Soluciones propias → catálogo del usuario. Distinta de Análisis → Solución Nutritiva (lab).
- ⏱️ **Pulso de riego en hidroponía** (\`hidro-pulso-riego-free.html\`, icono ⏱️ login/dashboard): calcula **L y minutos** del pulso. **1 Contenedor/sustrato:** V (L), % ATD (agua disponible; catálogo coco/perlita/lana… o sustrato propio), % agotamiento permitido, % drenaje (lavado de sales). Ideal: reponer agotamiento de ATD + añadir drenaje. **2 Sistema:** macetas, goteros/maceta, caudal L/h. **Fórmulas:** \`L_neto = V × (ATD%/100) × (agotamiento%/100)\`; \`L_pulso = L_neto ÷ (1 − drenaje%/100)\`; \`min = (L_pulso ÷ (goteros × L/h)) × 60\`; \`L_total = L_pulso × macetas\`. Orientativo (validar drenaje real en campo). Persistencia \`nutriplant_free_hidro_pulso_riego_v1\`. No guarda en el proyecto nube.
...
- 🌤️ **Pronóstico agroclimático** (login/dashboard, icono 🌤️; URL \`/pronosticoclimatico/\`): herramienta gratuita de lectura climática por punto (mapa/GPS/coords) + Kc (manual o FAO). Muestra ~7 d histórico + ~7 d pronóstico: T mín/máx, HR, rocío, Rad máx W/m², VPD mín/máx, ETo, ETc (= ETo×Kc), lluvia; gráfica con horas VPD por bandas (&lt;0.5 / 0.5–1.5 / &gt;1.5) + lluvia/ETo/ETc. **No es** la pestaña Clima del proyecto ni el VPD 🌡️ ni las Ventanas de aplicación foliar 🍃. La consulta gratis no crea alerta automática. Desde la herramienta se puede **solicitar alerta agroclimática semanal** (1 predio/persona): folio WhatsApp → revisión admin → si se aprueba, correo dominical 17:00 zona del predio con enlace personal (\`?token=\`) a reporte + PDF. Cambios permanentes de Kc/coords: WhatsApp/admin (no autosave al explorar el mapa en modo personal). Distinto de suscripción NutriPlant PRO ($49/5 meses).
- 🍃 **Ventanas de Aplicación Foliar** (login/dashboard, icono 🍃): **1 zona/lote**. Matriz **24 h** ~3 d (Open-Meteo) T, HR, viento, DPV aire y lluvia. **Sweet spot (muy favorable):** T 15–25 °C, HR 50–80 %, viento 2–8 km/h (en US customary se muestra °F y mph; física interna SI). Banda publicación más amplia T 18–28 / HR &gt;60 / viento 3–12. Semáforo 5 colores; factor limitante. Noche incluida (inversión/rocío). **UI ES/EN** + métrico/US. Persistencia \`nutriplant_free_ventanas_foliar_v1\`. Orientativo; no sustituye etiqueta. **≠** VPD puntual **≠** Pronóstico agroclimático **≠** análisis foliar DOP.
...
- **Ácido y auto-cálculo:** con análisis vinculado, dosis = mL/m³ × m³ ÷ 1000 → **L** (ácidos líquidos) o **kg** (cítrico polvo = L equiv. × densidad); entra **primero** en tanque **C** (modo producto); aporte elemental (N/P/S según ácido; **cítrico = 0**; nítrico aporta N-NO₃) resta del faltante antes de sales. El usuario también puede fijar **ppm de un elemento** en la celda del nutriente y se calcula el volumen/masa; debajo se muestra cuántos meq/L de alcalinidad (HCO₃⁻+CO₃²⁻) neutralizaría esa dosis. UI: etiqueta «Traer de análisis» + desplegable «Seleccionar análisis…» (al elegir se cargan ppm + dosis de ácido).
- **Extracto de pasta (al final del cálculo, tras aporte total):** «Traer de análisis» desde Análisis → Extracto de pasta. **No resta del faltante** (el agua sí). Compara pasta vs **objetivo**; si pasta > objetivo → bajaría sugerida = (pasta − objetivo) × **f** (f editable: 0,25 / 0,30 / 0,50 u otro 0–1). Steiner (1961) = equilibrio iónico de la solución (meq/L, triángulos); **no** prescribe restar pasta 1:1. Botón **Aplicar sugerencia al objetivo** baja ppm/meq **una vez por análisis** (luego «Permitir aplicar otra vez»); no vuelve a restar la pasta sola. Tras aplicar → revisar fertilizantes / Propuesta automática. Manual: \`analisis-extracto-pasta\` + \`hidroponia-solucion-por-etapa\`.
...
- Sin m³/ha de riego en la etapa (capturados en Programa) no hay ppm/meq en gráficas iónicas. Dinámica no vuelve a pedir la lámina; el selector de etapa muestra la del periodo.
...
/** Resumen compacto de hidroponia.cycleProgram para el prompt del chat. */
function summarizeHydroCycleProgramForContext(cp) {
  if (!cp || typeof cp !== 'object' || !Array.isArray(cp.stages) || !cp.stages.length) return '';
  const meqKeys = ['N_NH4', 'N_NO3', 'P', 'S', 'K', 'Ca', 'Mg'];
  const microKeys = ['Fe', 'Mn', 'B', 'Zn', 'Cu', 'Mo'];
  const name = String(cp.programName || '').trim();
  const activeId = cp.activeStageId || null;
  let out = '--- PROGRAMA DEL CICLO (hidroponia.cycleProgram; multi-etapa; ≠ Solución por etapa del diseño activo) ---\n';
  if (name) out += `Nombre del programa: ${name}\n`;
  out += `Etapas (${cp.stages.length}): ${cp.stages.map(function (s) { return s.name || s.id || '—'; }).join(', ')}\n`;
  const maxStages = 10;
  cp.stages.slice(0, maxStages).forEach(function (st) {
    const meq = st.meq || {};
    const ppm = st.ppm || {};
    const isActive = activeId && st.id === activeId;
    const meqPart = meqKeys
      .map(function (k) { return (Number(meq[k]) || 0) !== 0 ? `${k}:${Number(meq[k]).toFixed(2)}` : null; })
      .filter(Boolean)
      .join(', ');
    const microPart = microKeys
      .map(function (k) { return (Number(ppm[k]) || 0) !== 0 ? `${k}:${Number(ppm[k]).toFixed(2)}` : null; })
      .filter(Boolean)
      .join(', ');
    const cl = Number(ppm.Cl) || Number(meq.Cl) || 0;
    const ce = Number(st.ce);
    out += `• ${st.name || st.id || 'Etapa'}${isActive ? ' [activa]' : ''}`;
    if (Number.isFinite(ce) && ce > 0) out += ` · CE ~${ce.toFixed(2)}`;
    if (meqPart) out += ` · meq/L ${meqPart}`;
    if (microPart) out += ` · micros ppm ${microPart}`;
    if (cl) out += ` · Cl ${cl.toFixed(1)}`;
    out += '\n';
  });
  if (cp.stages.length > maxStages) out += `… y ${cp.stages.length - maxStages} etapa(s) más.\n`;
  out += 'PDF: sección opcional hydroCycle / «Programa del ciclo». Admin: bloque Programa del ciclo en la ficha del proyecto.\n\n';
  return out;
}
...
class NutriPlantChat {
  ...
  mapSectionToModule(sectionName) {
    ...
    if ((key.includes('solucion nutritiva') || key.includes('nutrient solution')) && !key.includes('analisis') && !key.includes('analysis')) return 'hidroponia';
...
- Acciones en cada tarjeta de proyecto: Abrir (carga ese proyecto como activo y permite usar las pestañas Ubicación, Enmienda, Nutrición Granular, Fertirriego, Solución Nutritiva / Hidroponía, Análisis, VPD, Reportes); Editar (cambiar nombre, cultivo, variedad, campo/sector); Duplicar (copia del proyecto, útil para variantes o nueva temporada); Eliminar (borra el proyecto y sus reportes en la nube si está conectado).
- Arquitectura NutriPlant PRO (para explicar cómo interactuar): Inicio = centro de proyectos. Una vez que el usuario abre un proyecto, la barra de pestañas permite ir a: Ubicación (polígono del predio), Enmienda (CIC y enmiendas cálcicas), Nutrición Granular (requerimiento + programa aplicaciones), Fertirriego (requerimiento + programa por semanas/meses + gráficas), Solución Nutritiva / Hidroponía (mismo módulo, tag hidroponia: solución por etapa + cálculo de fertilizantes), Análisis (suelo, foliar, agua, solución nutritiva de laboratorio, extracto de pasta, fruta), VPD (calculadoras déficit de presión de vapor), Reportes (generar PDF). En la barra superior del dashboard hay además calculadoras globales: 💧 Diseño de solución nutritiva (didáctico), ⏱️ Pulso de riego en hidroponía, 🔗 Interacciones y movilidad nutrimental, 🪨 Agua en suelo y textura, 🌧️ Lámina de riego, 📈 Rendimiento hídrico (ISH), 🍃 Ventanas de Aplicación Foliar, 🎯 Uniformidad de riego, 🧂 Solubilidad e índice salino (ver MANUAL CALCULADORAS PRO). Cada pestaña trabaja con los datos del proyecto activo; no hay que crear proyecto por cada pestaña. Si preguntan "por dónde empiezo", sugiere: crear/abrir proyecto en Inicio → luego Ubicación si tendrá mapa, Enmienda si tiene análisis de suelo, o la pestaña que corresponda a su objetivo (granular, fertirriego, hidroponía, etc.).`,
      enmienda: `
...
- **Aporte por agua · UI:** etiqueta «Traer de análisis» + desplegable «Seleccionar análisis…» (al elegir un reporte de Análisis → Agua se cargan kg/ha y la leyenda de ácido). Los kg/ha del agua en Programa = ppm × lámina total del ciclo (suma de etapas); si aún no hay lámina, usa el volumen de referencia del análisis. El ácido del programa = mL/m³ × lámina de cada etapa (L/ha); al elaborar el programa entra primero y su N/P/S resta del faltante. Sin análisis en el proyecto, el desplegable avisa «Sin análisis de agua…».
- **Costos (obligatorio):** precio canónico **USD/t métrica** por material (perfil/overrides; solubles compartidos con Hidroponía). Por columna: kg producto/ha = suma de dosis (si líquido: L/ha × densidad) → costo = kg/ha × (USD/t ÷ 1000) → total programa en **USD/ha** (o USD/acre en US customary). Sin precio → 0 / «—». No inventar precios de mercado.
...
- **Costos (obligatorio):** por material en una aplicación, kg producto/ha = dosis kg/ha × (% TM del material ÷ 100); costo = kg/ha × (precio USD/t ÷ 1000). Total aplicación / programa en **USD/ha** (USD/acre si US customary). Precio desde catálogo/overrides del usuario; sin precio → «—». No inventar precios.
- Contrastar plan de K/Ca/Mg contra diagnóstico de suelo para evitar excesos.`,
      hidroponia: `
- Hidroponía (en menú Proyecto se llama **Solución Nutritiva**; mismo módulo, tag interno **hidroponia** — no es un tag nuevo) tiene **dos subsecciones** en la pestaña + un **Programa del ciclo** aparte: (1) Solución por etapa: etapas, meq/L, **tabla % meq**, ppm, diagrama ternario; N total = N-NO₃⁻ + N-NH₄⁺. Catálogo de soluciones (Steiner/Hoagland/… + propias); botón vacío «Elegir» abre el catálogo. (2) Cálculo de fertilizantes: objetivo (ppm), análisis de agua (ppm), requerimiento = objetivo − agua; volumen (m³), tanque (L), inyección (L/m³); fertilizantes, dosis, tanques A–E, **meq/L + % meq del aporte** y leyenda N/Cl. (3) **Programa del ciclo** (botón en Solución Nutritiva → modal \`hidro-solucion-free\`): plan multi-etapa del cultivo (meq/ppm por fase, gráficas, catálogo); se persiste en **\`hidroponia.cycleProgram\`**; se ve en admin; en Reportes PDF se puede marcar la sección **Programa del ciclo** (\`hydroCycle\`) — **no** sustituye ni es lo mismo que «Solución por etapa» del diseño activo.
- No confundir con **Análisis → Solución Nutritiva** (🧪, reportes de laboratorio). Esta pestaña de Proyecto es diseño/cálculo; el análisis de lab es otra subpestaña.
...
- **Extracto de pasta en Cálculo de fertilizantes (obligatorio si preguntan):** al final del cálculo, «Traer de análisis» desde Extracto de pasta. **No resta** como el agua. Diff = pasta − objetivo; bajaría = exceso × f (f editable). **Aplicar sugerencia** → baja el objetivo 1× por análisis; no doble-resta. Steiner = equilibrio iónico, no fórmula de resta 1:1 de pasta. Tras aplicar, recalcular ferts.
- **Leyenda de ácido y volumen (obligatorio):** (1) ácidos del análisis / catálogo NutriPlant: **HNO₃ 55%** (11,6 meq/mL; **aporta N-NO₃**; líquido → UI en **mL/m³ y L**), H₂SO₄ 98% (36,7; aporta S; líquido), H₃PO₄ 75% (12,0) o 85% (14,6; aporta P; líquido), **Ácido Cítrico Anhidro 99.5%** (**polvo soluble**; 25,9 meq/mL; C₆H₈O₇ triprótico; densidad ~1,665 kg/L solo para equivalencia; **solo acidifica — no aporta N, P ni K**; UI primaria en **g/m³ o kg** métrico / **oz o lb** US customary; mL/L solo como *equiv. vol.*); (2) meq necesarios = (HCO₃⁻ + CO₃²⁻) − residual (defecto 1 meq/L); (3) mL/m³ = meq × 1000 / meqPerMl; (4) L o kg del m³ del análisis y del volumen de hidroponía; (5) aviso si m³ coinciden (~0,01); (6) revisar dosis. Misma lógica en **Análisis → Agua**, **acondicionamiento gratis 💦**, **admin**, **PDF** (EN + US units si aplica).
...
- Extractor PDF de suelo: densidad aparente también viene abreviada en labs MX («Dens. Aparente», «¹Dens. Aparente», DA, Dap). El ¹/1 es nota al pie, no el valor; el número real va con g/cm³ (ej. 1.32). Si el valor extraído no queda entre 0.8 y 1.4 g/cm³ (o no aparece), la plataforma pone 1 y el usuario lo puede editar.
- Fertilidad del suelo (lógica que el chat debe entender): El usuario ingresa valores de laboratorio (Nivel) y la plataforma muestra Ideal (referencia) y kg/ha (diferencia). En kg/ha solo se considera el suelo que las raíces aprovechan en la profundidad indicada. Un ppm de P/micros/B no es universal: hay que conocer el extractante (selectores en la cabecera). Valores ideales: (1) K, Ca y Mg: desde la CIC del mismo análisis (sección Cationes), meq ideal = CIC × fracción (K 5 %, Mg 13 %, Ca 70 %) y ppm = meq × factor (K ×391, Mg ×121,5, Ca ×200,4). No dependen del extractante de P/micros. (2) P: Bray 40, Olsen 25, Merich 40. (3) Fe/Mn/Zn/Cu: un selector DTPA 20/20/3/1,5 o Merich 50/20/3/2 (Otro no pisa). (4) B: agua caliente 1 o Merich 1,2 (Otro no pisa; B no va con DTPA). (5) Resto (MO, N-NO3, Na, S, Mo, Al): referencias generales. Al cambiar método la app pisa esos ideales; el usuario puede editarlos (se guardan en el reporte). No compares extractantes distintos 1:1. Cálculo kg/ha: factor = 0,1 × profundidad (cm) × densidad aparente (g/cm³) × (% suelo explorado / 100); kg/ha = (nivel − ideal) × factor; negativo = déficit, positivo = exceso. Si pregunta de dónde salen los ideales: fórmula CIC para K/Ca/Mg + defaults por método para P/micros/B; si la fila Ideal difiere, usar lo guardado en pantalla.
- Cationes intercambiables y CIC: **meq/100g** (= **cmol_c/kg**, mismo número; carga catiónica por masa de suelo). Ca, Mg, K, Na, Al, H; CIC = suma. Saturación (%) = (meq catión / CIC) × 100. Ideal de saturación (pastilla bajo cada %): Ca 65–75, Mg 10–15, K 3–7, Na 0–1, Al 0–1, H 0–10. Relaciones entre cationes (desde meq): Ca/Mg = 6, Mg/K = 3.5, (Ca+Mg)/K = 18, Ca/K = 14 — pastilla «id.» bajo cada cálculo; semáforo ±5 % de esa referencia. Mismo criterio en comparar, admin y PDF. En fertilidad, el ideal en ppm de K, Ca y Mg se calcula con la CIC y objetivos fijos K 5 %, Mg 13 %, Ca 70 % más factores meq→ppm. Botón "Recargar valores ideales de referencia" aplica ideales generales y recalcula K, Ca, Mg desde CIC si existe.
- Solución Nutritiva (lógica que el chat debe entender): Cada reporte tiene valores de análisis (nivel/resultado en meq/L y ppm), una columna **% meq** junto a meq (catión/suma de cationes; anión/suma de aniones; todos los iones de la tabla; ≠ triángulo Steiner) y fila **Total** = Σ meq del grupo + 100, rangos de referencia (ej. Ca 140–220, Mg 40–70, K 180–300, SO4 60–110, PO4 30–60, NO3 140–200 ppm; micronutrientes Fe 1.5–3, Mn 0.3–1, etc.) y una columna Ideal editable por el usuario. El usuario puede dejar el ideal vacío o definir su propio valor ideal por parámetro (distinto al rango de referencia). Diferencia = valor actual (nivel) − ideal; se muestra en la tabla: (−) falta (por debajo del ideal), (+) exceso (por encima del ideal). Semáforo: si Ideal está vacío, la luz es vs la franja Ref.; si el usuario llena Ideal, la luz es vs ese número (±10 % verde, como DOP). Ejemplo: nivel 79 e ideal 80 = verde, aunque Ref. sea 180–300. El chat debe ver tanto los valores que tiene el usuario como los ideales (los que haya guardado el usuario o referencia) e interpretar la diferencia para apoyar el diagnóstico y ajustes del programa de nutrición.
- Extracto de Pasta (lógica que el chat debe entender): Igual que Solución Nutritiva: cada reporte tiene nivel (meq/L y ppm), **% meq** junto a meq (mismo criterio: % del total de cationes / % del total de aniones; fila Total = Σ meq + 100), rangos de referencia (Ca 150–220, Mg 40–70, K 200–300, Na &lt;50, NO3 150–200, PO4 30–60, SO4 60–110, Cl &lt;70, HCO3 &lt;120, CO3 0 ppm; micronutrientes con ref.) e Ideal editable por el usuario. Diferencia = nivel − ideal; (−) falta, (+) exceso. Semáforo: vacío → vs Ref.; con Ideal → ±10 % de ese número (79 vs 80 = verde aunque Ref. sea 200–300). El usuario puede agregar o cambiar ideales por parámetro; el chat debe usar los valores e ideales que tenga el reporte (por defecto o añadidos por el usuario) y entender la lógica de la diferencia para interpretar déficits y excesos. Además hay ratios (NO3/K, K/Ca, K/Mg, Ca/Mg, Ca/Na) con rangos ideales de referencia.
- **Extracto de pasta ↔ Hidroponía (Cálculo):** no confundir con restar agua. En Cálculo de fertilizantes se puede traer el extracto al final: referencia de rizósfera vs objetivo; si pasta &gt; objetivo sugiere (pasta − objetivo) × f (f editable 25/30/50 % u otro). Botón Aplicar baja el objetivo una vez por análisis. Steiner guía equilibrio iónico de la solución, no resta 1:1 de pasta. Manual público: capítulos extracto de pasta + hidroponía solución por etapa.
- Análisis de Agua: lista "Reportes en este proyecto" y "+ Agregar análisis"; por reporte: volumen de referencia (Fertirriego = lámina total del ciclo; Hidroponía = m³ de solución o agua a inyectar), título, fecha, Eliminar. Secciones: CE/RAS/pH, Cationes, Aniones, Micronutrimentos, Ácido para neutralizar. Conversión meq/L ↔ ppm automática; uso del agua en Fertirriego e Hidroponía para restar aporte del agua del requerimiento.
...
- Subpestaña **Rendimiento hídrico (ISH)** (Clima, después de Lluvia/Riego): \`ISH = 100×[1−Σ(D+Fp·E)/ΣETc]\` semanal (máx. 52). Misma física que la herramienta gratis 📈. ET₀/lluvia satélite o manual; Kc sync; Fp; riego mm↔m³/ha + % efectivo; opcional traer riego de Lluvia/Riego si periodo = 7 d. Curva de techo por agua (solo baja). Persistencia \`climateAnalysis.ish\`. **≠** balance de periodo (m³ a aplicar).`,
      ubicacion: `
...
- Calculadoras globales en barra del dashboard (cualquier pestaña): 💧 Diseño de solución nutritiva (didáctico), ⏱️ Pulso de riego en hidroponía (V×ATD×agotamiento÷(1−drenaje); macetas/goteros → L y min), 🔗 Interacciones y movilidad, 🪨 Agua en suelo y textura, 🌧️ Lámina de riego y balance hídrico, 📈 Rendimiento hídrico (ISH), 🍃 Ventanas de Aplicación Foliar (1 lote, matriz horaria T/HR/viento/DPV/lluvia; UI ES/EN + °C/°F y km/h/mph), 🎯 Uniformidad de riego (lotes + DU/CU/EU; UI ES/EN + métrico/US), 🧂 Solubilidad e índice salino — ver MANUAL CALCULADORAS PRO.
...
- Las herramientas gratis 🍃 Ventanas de Aplicación Foliar y 🎯 Uniformidad de riego siguen el idioma y el sistema de unidades del perfil: T/viento en °C·km/h o °F·mph; lámina/marco/dosis/presión EU en métrico o US customary. La física interna permanece SI.
...
- Conoces la arquitectura de NutriPlant: módulos (Inicio, Ubicación, Enmienda, Nutrición Granular, Fertirriego, Hidroponía / Solución Nutritiva, Análisis, VPD, Reportes). **Hidroponía y Solución Nutritiva (Proyecto, icono 💧) son el MISMO módulo** (tag interno \`hidroponia\`; no se separa). Subpestañas de Análisis (Suelo, Solución Nutritiva 🧪 laboratorio, Extracto de Pasta, Agua, Foliar/DOP, Fruta/ICC) y cómo se relacionan (p. ej. Suelo→Enmienda, Agua→Fertirriego/Hidroponía, Foliar/Suelo/Fruta→diagnóstico integrado).
- Los datos que te pasamos son del MISMO proyecto en su totalidad: incluyen TODAS las secciones que el usuario tenga guardadas (Enmienda, Fertirriego, Granular, Hidroponía / Solución Nutritiva de Proyecto, Análisis de Suelo, Foliar, Fruta, Agua, Solución Nutritiva de laboratorio, Extracto de Pasta, Extracción por etapa 📊, etc.), aunque el usuario esté en otra pestaña. Por ejemplo: si está en Fertirriego y te pregunta por su análisis foliar o por su suelo, tienes esos datos en el bloque "DATOS DEL PROYECTO" y debes usarlos para responder e interactuar con él.
...
- **Hidroponía / Solución Nutritiva (Proyecto)**: concentraciones y aportes de fertilizantes son SIEMPRE en forma ELEMENTAL (%, ppm por elemento). No hay modo óxido en hidroponía. Tag de contexto: **hidroponia**.
- **Orden para determinar el marco de contexto** (usa este orden cuando pregunten por *sus* datos o por interpretar *su* pantalla): (1) **Mención explícita en el mensaje**: si el usuario dice "hidroponía" → siempre el módulo hidroponia. Si dice "solución nutritiva": si el módulo activo es hidroponia, o habla de etapas / cálculo de fertilizantes / catálogo Steiner / tanques A–E → marco hidroponia; si está en Análisis o habla de reportes de laboratorio, ideales, diferencia vs rango → bloque ANÁLISIS SOLUCIÓN NUTRITIVA. "foliar", "análisis de agua", "enmienda", "suelo", "extracto de pasta", "cmol", "meq", etc. → el marco es el de ese bloque. (2) **Módulo activo**: en "DATOS DEL PROYECTO" aparece "Módulo activo: X"; si X = hidroponia → marco solución (ppm/meq/L); si X = enmienda → marco suelo inicial (**meq/100g = cmol_c/kg** misma cifra); si X = analisis → ver (3). (3) **Dentro de Análisis**: en el bloque "ANÁLISIS (pestaña actual)" se indica qué subpestañas tienen datos "en pantalla" y "Reporte seleccionado"; usar la subpestaña que el usuario nombre o la que tenga reporte seleccionado, o inferir por palabras del mensaje (ej. "micros en solución" en Análisis → laboratorio de solución; "ppm foliar" o "DOP" → foliar). (4) **Marco = unidades de ese bloque**: solución/hidroponía → ppm y meq/L; foliar → mg/kg en tejido; suelo (CIC/cationes) → meq/100g = cmol_c/kg (misma cifra); agua/solución en cmol/L del usuario → convertir ×10 a meq/L; granular/fertirriego → kg/ha. Una vez determinado el marco, referencias/ideales/literatura en las MISMAS unidades; no mezcles (ej. no des rangos foliares si el marco es solución, ni al revés). Si preguntan conversión cmol/meq/mmol → MANUAL UNIDADES meq / cmol / mmol.
...
MANUAL RADAR SATELITAL (NDVI/NDMI + Lectura — siempre disponible; datos del proyecto en bloque RADAR / LECTURA si existen):
${radarManual}
...
- Solución Nutritiva (Análisis 🧪) vs Extracto de Pasta: son dos subpestañas distintas dentro de Análisis. Cada una tiene su propia lista de reportes, valores e ideales. No cruces ni mezcles datos entre ellas: si preguntan por **análisis** de solución nutritiva (laboratorio, ideales, diferencia), usa solo el bloque "ANÁLISIS SOLUCIÓN NUTRITIVA"; si preguntan por extracto de pasta, usa solo el bloque "ANÁLISIS EXTRACTO DE PASTA". Si preguntan por la pestaña de Proyecto **Solución Nutritiva** (💧, hidroponía, etapas, cálculo de fertilizantes), usa el bloque HIDROPONÍA — mismo tag \`hidroponia\`.
...
- En Cálculo de fertilizantes (Hidroponía): objetivo, análisis de agua (incl. Cl⁻; selector **Traer de análisis**), requerimiento, volumen/tanque/inyección, tanques A/B/C, aporte total ppm, leyenda % sobre meq/L (N-NO₃ vs N-NH₄ y N-NO₃ vs Cl, con y sin agua). **Leyenda de ácido:** ácido del análisis, mL/m³, L según m³ del análisis y L según volumen de hidroponía, aviso de coincidencia de volúmenes; misma leyenda en admin/PDF (EN si el reporte está en inglés). **Extracto de pasta (bloque final):** Traer de análisis → comparar vs objetivo; bajaría = exceso × f (editable); Aplicar 1×/análisis (no resta como agua). Orden nutrientes: macros, micros, Cl al final. Todo elemental; óxidos → calculadora óxido↔elemental. Para antagonismos/movilidad/solubilidad → MANUAL CALCULADORAS PRO (🔗 🪨 🧂).
- Si preguntan por 💧 Diseño de solución nutritiva (barra dashboard), 🔗 Interacciones, 🪨 Agua en suelo y textura, o 🧂 Solubilidad e índice salino: usa el MANUAL CALCULADORAS PRO (no guardan datos en el proyecto; la 💧 didáctica es distinta de Solución Nutritiva / Hidroponía PRO por etapa).
...
- Si es "detallada": explica con más profundidad técnica, sin perder claridad.`;
  ...
  isCalculationOrLogicQuestion(message) {
    ...
    const keywords = [
      ...
      'lectura satelital', 'lectura', 'quincenal', 'histórico satelital', 'historico satelital'
    ];
...
- Diferencia claramente:
  ...
  detectCurrentModule() {
    ...
    if ((title.includes('solución nutritiva') || title.includes('solucion nutritiva') || title.includes('nutrient solution')) && !title.includes('análisis') && !title.includes('analisis') && !title.includes('analysis')) return 'hidroponia';
  ...
  getRadarNdviContext(projectId, projectData) {
    ...
    const lectura = projectData?.location?.lecturaSatelital;
    if (lectura && typeof lectura === 'object') {
      lines.push('');
      lines.push('--- LECTURA SATELITAL (histórico por periodos) ---');
      lines.push(
        `Parámetros: frecuencia=${lectura.frequency || '—'}; periodos=${lectura.periods != null ? lectura.periods : '—'}; fecha final=${lectura.endDate || '—'}.`
      );
      const rows = Array.isArray(lectura.rows) ? lectura.rows.slice() : [];
      ...
      if (rows.length) {
        ...
        rows.forEach(function (r) {
          const parts = [
            r.label || ('P' + r.index),
            r.status || '—',
            r.ndvi_mean != null ? 'NDVI=' + r.ndvi_mean : null,
            r.ndmi_mean != null ? 'NDMI=' + r.ndmi_mean : null,
            r.vpd_mean != null ? 'VPD=' + r.vpd_mean + ' kPa' : null,
            r.et0_sum != null ? 'ET0=' + r.et0_sum + ' mm' : null,
            r.rain_sum != null ? 'lluvia=' + r.rain_sum + ' mm' : null,
            r.riego_m3 != null ? 'riego=' + r.riego_m3 + ' m³' : null,
            r.lookback_expanded ? 'ampliada-a-mes' : null
          ].filter(Boolean);
          lines.push('  · ' + parts.join('; '));
        });
        lines.push(
          'Interpretar tendencias NDVI/NDMI vs VPD, ET₀, lluvia y riego; no atribuir causa única. Costo Lectura = fijo por consulta: 3 créditos (4 si el predio es >30 ha).'
        );
      } else {
        lines.push('Sin filas de Lectura aún: el usuario puede generar el histórico en Radar Satelital → Lectura Satelital.');
      }
    }
  ...
  getLiveFertirriegoBlocks() {
    const out = { subsection: '', cultivo: '', rendimiento: '', tableSummary: '', ionicEqSummary: '', macroIonicSummary: '', sourceShareSummary: '', waterContributionSummary: '', baseContributionSummary: '' };
    ...
    if (real.length) rows.push('Requerimiento real (kg/ha): ' + real.join(', '));
    if (rows.length) out.tableSummary = rows.join('\n');
    const eqBlock = document.getElementById('fertiIonicEqBlock');
    if (eqBlock && eqBlock.textContent) {
      out.ionicEqSummary = eqBlock.innerText.replace(/\s+/g, ' ').trim().slice(0, 900);
    }
  ...
  getLiveHidroponiaBlocks() {
    const out = { subsection: '', volume: '', objectiveSummary: '', waterSummary: '', missingSummary: '', solutionMeqTable: '', solutionPercentTable: '', solutionPpmTable: '', nitrogenSummary: '', triangleInfo: '', fertMeqSummary: '', fertLegendSummary: '', acidVolumeLegend: '' };
    ...
    if (tab === 'hidro-solucion') {
      ...
      const acidBox = document.getElementById('hydroAcidSummary');
      ...
      if (acidBox && acidBox.textContent && acidBox.textContent.trim()) {
        out.acidVolumeLegend = acidBox.textContent.replace(/\s+/g, ' ').trim().slice(0, 700);
      }
      try {
  ...
  getLiveAnalisisSueloBlocks() {
    ...
    if (wrap && wrap.style.display !== 'none') {
      ...
      out.soilFertilityLogic = {
        ...
        microMethod: (document.getElementById('soil-fertility-microMethod') || {}).value || null,
        bMethod: (document.getElementById('soil-fertility-bMethod') || {}).value || null,
  ...
  getProjectContext() {
    ...
    try {
      ...
      if (snapshot.module === 'fertirriego') {
        ...
        if (liveFerti.cultivo || liveFerti.rendimiento || liveFerti.tableSummary || liveFerti.subsection || liveFerti.ionicEqSummary || liveFerti.macroIonicSummary || liveFerti.sourceShareSummary || liveFerti.waterContributionSummary || liveFerti.baseContributionSummary) {
          ...
          if (liveFerti.ionicEqSummary) context += `Zona de equilibrio iónico (apoyo para armar el programa; no va en PDF; rangos % fijos; kg y % actuales se mueven con el req): ${liveFerti.ionicEqSummary}\n`;
      ...
      if (hydro) {
        context += '--- HIDROPONÍA / SOLUCIÓN NUTRITIVA (Proyecto; tag hidroponia — Solución por etapa + Cálculo de fertilizantes + Programa del ciclo) ---\n';
        context += 'En el menú este módulo se llama Solución Nutritiva (💧). Interno: hidroponia. Distinto de Análisis → Solución Nutritiva (laboratorio). Subsecciones relacionadas: Solución por etapa define el objetivo (ppm/meq por etapa); Cálculo de fertilizantes usa ese objetivo, resta el aporte del agua, y calcula fertilizantes y dosis. Aparte: botón «Programa del ciclo» abre plan multi-etapa (modal); se guarda en hidroponia.cycleProgram y puede ir al PDF (hydroCycle).\n';
        ...
        context += 'Subsección Cálculo de fertilizantes: objetivo (ppm), análisis de agua (ppm, incl. Cl⁻; selector Traer de análisis), requerimiento total (ppm), volumen, tanque, inyección; catálogo precargado + personalizado (% elemental, incl. Cl). Leyenda bajo pendiente: % sobre meq/L (N-NO₃/N-NH₄ y N-NO₃/Cl) solo fertilizantes y solución final (fert+agua). Leyenda de ácido: ácido del análisis, mL/m³, L del análisis vs L del volumen de hidroponía, aviso si los m³ coinciden o no (también en admin/PDF). Extracto de pasta (bloque final): Traer de análisis → comparar vs objetivo; no resta como agua; bajaría = exceso × f (editable); Aplicar sugerencia 1×/análisis.\n';
        if (hydro.waterAnalysisId) context += `Análisis de agua vinculado (waterAnalysisId): ${hydro.waterAnalysisId}\n`;
        if (hydro.acidDoseSummary && typeof hydro.acidDoseSummary === 'object') {
          const ads = hydro.acidDoseSummary;
          if (ads.hasAcid) {
            context += `Resumen ácido guardado: ${ads.acidNameEs || ads.acidId || '—'}; ${Number(ads.mlPerM3 || 0).toFixed(2)} mL/m³; volumen análisis ${ads.analysisVolumeM3 != null ? ads.analysisVolumeM3 : '—'} m³; volumen hidro ${ads.hydroVolumeM3 != null ? ads.hydroVolumeM3 : '—'} m³; L análisis ${ads.analysisTotalLiters != null ? Number(ads.analysisTotalLiters).toFixed(2) : '—'}; L hidro ${ads.totalLiters != null ? Number(ads.totalLiters).toFixed(2) : '—'}; volúmenes coinciden: ${ads.volumesMatch ? 'sí' : 'no'}\n`;
          } else if (ads.waterAnalysisId) {
            context += 'Hay análisis vinculado pero sin ácido válido calculado (revisar HCO₃/CO₃/residual/ácido en Análisis → Agua).\n';
          }
        }
        ...
        const cycleCtx = summarizeHydroCycleProgramForContext(hydro.cycleProgram);
        if (cycleCtx) context += cycleCtx;
        else context += 'Programa del ciclo: sin etapas guardadas aún (botón «Programa del ciclo» en Solución Nutritiva; se guarda en hidroponia.cycleProgram).\n\n';
      }
      ...
      if (snapshot.module === 'hidroponia') {
        ...
        if (liveHydro.subsection || liveHydro.volume || liveHydro.objectiveSummary || liveHydro.waterSummary || liveHydro.missingSummary || liveHydro.nitrogenSummary || liveHydro.triangleInfo || liveHydro.fertMeqSummary || liveHydro.fertLegendSummary || liveHydro.acidVolumeLegend) {
          context += '--- BLOQUES HIDROPONÍA / SOLUCIÓN NUTRITIVA (PANTALLA ACTUAL) ---\n';
          ...
          if (liveHydro.subsection === 'Cálculo de fertilizantes') {
            ...
            if (liveHydro.acidVolumeLegend) context += `Leyenda ácido / volumen (pantalla; mL/m³, L análisis vs L hidro, coincidencia de m³): ${liveHydro.acidVolumeLegend}\n`;
      ...
      if (snapshot.module === 'reportes') {
        ...
        context += 'Pestaña para generar y gestionar reportes PDF. Cómo hacerlo: botón "Generar Nuevo Reporte PDF" → se abre un modal donde el usuario selecciona qué secciones incluir (Ubicación, Enmiendas, Nutrición granular, Fertirriego, Solución Nutritiva / Hidroponía, **Programa del ciclo** / hydroCycle, Análisis de lab, Extracción por etapa, VPD); marca las que quiera y genera; el PDF se crea solo con esas secciones. «Programa del ciclo» usa hidroponia.cycleProgram (multi-etapa) y es independiente de la sección Hidroponía (diseño activo + fertilizantes). También se puede abrir el mismo flujo desde la sección de enmiendas. Lista de reportes generados: cada uno con Descargar (PDF) y Eliminar; se guardan en el proyecto y se sincronizan a la nube.\n';
      ...
      if (snapshot.module === 'analisis') {
        ...
        if (liveSuelo.visible) {
          context += 'ORIGEN "Ideal (referencia)" en Fertilidad (NutriPlant): K/Ca/Mg ppm por defecto = CIC (suma meq/100g Ca+Mg+K+Na+Al+H del mismo reporte) × saturación objetivo (K 5 %, Mg 13 %, Ca 70 %) → meq ideal; luego ppm = meq×(K 391, Mg 121,5, Ca 200,4). Un ppm de P/micros/B no es universal: leer pMethod, microMethod y bMethod. P: Bray 40 / Olsen 25 / Merich 40. Fe/Mn/Zn/Cu: DTPA 20-20-3-1,5 o Merich 50-20-3-2 (un selector; Otro no pisa). B: agua caliente 1 o Merich 1,2 (selector propio). Al cambiar método se pisan esos ideales; si el usuario editó, usar lo de pantalla. No comparar extractantes 1:1. MO, N-NO3, Na, S, Mo, Al: referencias fijas (botón recargar). kg/ha diff = (lab−ideal)×0,1×prof(cm)×densidad aparente(g/cm³)×(%raíz/100).\n';
          if (liveSuelo.reportTitles.length === 0) {
            ...
            if (liveSuelo.currentId && liveSuelo.currentTitle) {
              ...
              if (liveSuelo.soilFertilityLogic) {
                ...
                if (L.microMethod) bits.push(`método Fe/Mn/Zn/Cu ${L.microMethod}`);
                if (L.bMethod) bits.push(`método B ${L.bMethod}`);
        ...
        if (liveAgua.visible) {
          if (liveAgua.reportTitles.length === 0) {
            ...
            if (liveAgua.currentId && liveAgua.currentTitle) {
              ...
              if (liveAgua.m3Riego) context += `Volumen de referencia (m³): ${liveAgua.m3Riego}. `;
              if (liveAgua.general) {
      ...
      const summariseSoilReport = (a) => {
        ...
        if (fert.microMethod) parts.push(`micros_método:${fert.microMethod}`);
        if (fert.bMethod) parts.push(`B_método:${fert.bMethod}`);
      ...
      const summariseAgua = (a) => {
        ...
        const parts = [];
        if (a.m3Riego != null && a.m3Riego !== '') parts.push(`vol_ref:${a.m3Riego}`);
